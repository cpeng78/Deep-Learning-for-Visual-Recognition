    ...
    def update(self, model):
        ...
        for idx, m in enumerate(model.modules):
            if hasattr(m, 'weight'):
                ...
                #############################################################################
                m.dw = self.momentum * self.grad_tracker[idx]['dw'] - self.learning_rate * m.dw
                m.weight += m.dw
                self.grad_tracker[idx]['dw'] = m.dw
                #############################################################################
                #                              END OF YOUR CODE                             #
                #############################################################################
            if hasattr(m, 'bias'):
                ...
                #############################################################################
                m.db = self.momentum * self.grad_tracker[idx]['db'] - self.learning_rate * m.db
                m.bias += m.db
                self.grad_tracker[idx]['db'] = m.db
                #############################################################################
                #                              END OF YOUR CODE                             #
                #############################################################################