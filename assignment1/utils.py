...
def load_mnist_trainval():
    ...
    train_label = label[:round(len(data)*0.8)]
    val_data = data[round(len(data)*0.8):]
    val_label = label[round(len(data)*0.8):]
    #############################################################################
    # TODO:                                                                     #
...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle:
        ...
        label = np.array(label)
    batch_indice = 0
    while batch_indice < len(label) - batch_size:
        batched_data.append(data[batch_indice: batch_indice + batch_size, :])
        batched_label.append(label[batch_indice : batch_indice + batch_size])
        batch_indice += batch_size
    batched_data.append(data[batch_indice : , :])
    batched_label.append(label[batch_indice : ])
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    ax2.set(xlabel='Epochs', ylabel='Accuracy', title='Accuracy Curve')
